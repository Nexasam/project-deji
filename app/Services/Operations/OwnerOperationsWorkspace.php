...
{
    public function build(Business $business, array $filters, ?array $propertyIds = null, ?string $assignedEmployeeId = null): array
    {
        $base = $business->operationalTasks()
            ->when($propertyIds !== null, fn ($query) => $query->whereIn('property_id', $propertyIds))
            ->when($assignedEmployeeId !== null, fn ($query) => $query->where('assigned_employee_id', $assignedEmployeeId));
        $tasks = (clone $base)->with(['property', 'booking.guest', 'checklistItems', 'attachments', 'assignedEmployee.businessMembership.user'])
