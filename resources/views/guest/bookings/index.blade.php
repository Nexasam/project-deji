...
<main class="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
    ...
    <div class="mt-8 grid gap-5">
        @forelse($bookings as $booking)
            ...
            @endphp
            <article class="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-lg">
                <a class="grid sm:grid-cols-[220px_1fr]" href="{{ route('guest.bookings.show', $booking) }}">
                    <div class="h-48 overflow-hidden bg-slate-100 sm:h-full"><img src="{{ $image }}" onerror="this.src='/image.png'" alt="{{ $booking->property->name }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105"></div>
                    <div class="p-5 sm:p-6">
                        <div class="flex flex-wrap items-start justify-between gap-3"><div><p class="text-xs font-bold uppercase tracking-wider text-orange-600">{{ $booking->reference }}</p><h2 class="mt-1 text-xl font-extrabold text-slate-950">{{ $booking->property->marketplaceListing?->public_title ?: $booking->property->name }}</h2><p class="mt-1 text-sm text-slate-500">{{ data_get($booking->property->address, 'city') }}, {{ data_get($booking->property->address, 'state') }}</p></div><span class="rounded-full px-3 py-1 text-xs font-extrabold {{ $statusClass }}">{{ str($status)->replace('_', ' ')->title() }}</span></div>
                        ...
                        <div class="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-xs"><span class="font-semibold text-slate-500">Payment: {{ str($booking->payment_status->value)->replace('_', ' ')->title() }}</span><span class="font-extrabold text-orange-600">View booking →</span></div>
                    </div>
                </a>
            </article>
        @empty
