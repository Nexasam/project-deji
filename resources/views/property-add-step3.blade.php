...
<body class="bg-gray-100 font-sans antialiased" x-data="{
    type: '{{ old('property_type', $property->property_type) }}',
    booking: '{{ old('booking_mode', $property->booking_mode ?? 'entire') }}',
    guests: {{ old('capacity', $property->capacity) }},
    bedrooms: {{ old('bedrooms', $property->bedrooms) }},
    bathrooms: {{ old('bathrooms', $property->bathrooms) }},
    size: '{{ old('floor_area_sqm', $property->floor_area_sqm) }}'
}">
<div class="min-h-screen flex flex-col">
    ...
    <header class="bg-white border-b border-gray-200">
        <div class="max-w-3xl mx-auto px-6 py-3 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <span class="text-xs text-gray-500">Step 3 of 10</span>
    ...
    <main class="flex-1 py-7 px-4">
        <form method="POST" action="{{ route('owner.properties.wizard.store', ['property'=>$property,'step'=>3]) }}" class="max-w-3xl mx-auto">
            @csrf
            <input type="hidden" name="property_type" x-model="type"><input type="hidden" name="booking_mode" x-model="booking"><input type="hidden" name="capacity" x-model="guests"><input type="hidden" name="bedrooms" x-model="bedrooms"><input type="hidden" name="bathrooms" x-model="bathrooms">
            ...
            <div class="mb-2">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <button type="button" @click="type = '{{ $t['val'] }}'"
            ...
            <div class="bg-white rounded-xl border border-gray-200 mt-8">
                ...
                <div class="px-6 pt-6 pb-4">
                    ...
                    <div class="flex border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <button type="button" @click="booking = '{{ $val }}'"
                ...
                <div class="border-t border-gray-100 px-6">
                    ...
                    <div class="flex items-center justify-between py-4 border-b border-gray-100">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="button" @click="bedrooms = Math.max(0, bedrooms-1)"
                                class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-700 hover:border-gray-500 text-lg font-light">−</button>
                            <span class="w-6 text-center text-sm font-semibold" x-text="bedrooms"></span>
                            <button type="button" @click="bedrooms++"
                                class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-700 hover:border-gray-500 text-lg font-light">+</button>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between py-4 border-b border-gray-100">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="button" @click="bathrooms = Math.max(0, bathrooms-1)"
                                class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-700 hover:border-gray-500 text-lg font-light">−</button>
                            <span class="w-6 text-center text-sm font-semibold" x-text="bathrooms"></span>
                            <button type="button" @click="bathrooms++"
                                class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-700 hover:border-gray-500 text-lg font-light">+</button>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between py-4">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="number" name="floor_area_sqm" x-model="size" placeholder=""
                ...
                <div class="flex items-center justify-between px-6 py-5 border-t border-gray-100">
                    <a href="{{ route('owner.properties.wizard.step', ['property'=>$property,'step'=>2]) }}" class="text-sm text-gray-500 hover:text-gray-700 transition-colors underline">Back</a>
                    <button type="submit"
