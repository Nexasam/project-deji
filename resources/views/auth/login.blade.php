...
<head>
    ...
    <title>Sign in - {{ config('app.name', 'Verified Shortlet') }}</title>
    ...
    <main class="grid min-h-screen lg:grid-cols-[minmax(360px,0.86fr)_minmax(540px,1.14fr)]">
        <section class="relative hidden overflow-hidden bg-slate-950 p-10 text-white lg:flex lg:flex-col xl:p-14">
            ...
            <div class="relative z-10 my-auto max-w-lg py-16">
                ...
                <p class="mt-6 max-w-md text-base leading-7 text-slate-200">Whether you are booking a stay, managing a property, supporting guests or overseeing the platform, Verified Shortlet keeps your work connected.</p>
        ...
        <section class="flex min-h-screen items-center justify-center px-5 py-8 sm:px-8 lg:px-12 xl:px-20">
            <div class="w-full max-w-md" x-data="{ showPassword: false }">
                ...
                @if (Route::has('register'))
                    <p class="mt-8 text-center text-sm text-slate-600">New to Verified Shortlet? <a href="{{ route('register') }}" class="font-extrabold text-orange-600 hover:text-orange-700">Create an account</a></p>
                @endif
